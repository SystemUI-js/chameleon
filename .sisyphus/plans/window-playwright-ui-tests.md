...
- [x] 1. Add Playwright tooling and Chromium-only scripts
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `package.json` 新增 `test:ui` 与 `test:ui:headed`，且 `yarn test` 保持不变
  - [ ] `playwright.config.ts` 只声明 `chromium` project，`testDir` 指向 `tests/ui`
  - [ ] `playwright.config.ts` 复用 `http://127.0.0.1:5673`，并通过 `yarn dev` 启动 web server
  - [ ] `npx playwright test --config=playwright.config.ts --list` 可成功列出测试，不报配置错误
...
- [x] 2. Create deterministic Window Playwright harness
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] 访问 `/playwright-window.html?fixture=default` 时仅渲染一个 `window-frame`，初始样式为 `left: 10px; top: 20px; width: 240px; height: 160px`
  - [ ] `drag-only` fixture 不渲染任何 `window-resize-*` handle，且标题栏仍存在
  - [ ] `min-clamp` 与 `max-clamp` fixture 各自渲染一扇窗口并暴露既有 selector
  - [ ] 未知 `fixture` 参数渲染 `[data-testid="fixture-error"]`，文本明确包含请求的 fixture 名称
...
- [x] 3. Add shared Playwright Window helpers and smoke coverage
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `tests/ui/window.helpers.ts` 提供 fixture 导航、frame metrics 读取、locator 拖拽 helper，供多个 spec 复用
  - [ ] `tests/ui/window.smoke.spec.ts` 使用 helper 打开 `default` fixture 并断言基线几何 `(10,20,240,160)`
  - [ ] `tests/ui/window.smoke.spec.ts` 包含 `unknown fixture shows explicit error` 用例，并断言 `[data-testid="fixture-error"]` 可见
  - [ ] helper 默认使用 `window-frame`、`window-title`、`window-content`、`window-resize-{dir}` 作为主 selector
...
- [x] 4. Add Window move Playwright coverage
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `window.move.spec.ts` 在 `default` fixture 中断言标题栏拖拽后 frame metrics 精确等于 `{ x: 30, y: 60, width: 240, height: 160 }`
  - [ ] 同一 spec 断言拖拽 `window-content` 后 frame metrics 维持 `{ x: 10, y: 20, width: 240, height: 160 }`
  - [ ] 所有 move 用例只通过现有 `data-testid` 完成定位
...
- [x] 5. Add Window resize direction matrix Playwright coverage
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `window.resize.spec.ts` 参数化覆盖全部 8 个方向句柄
  - [ ] 每个方向都断言精确 `x/y/width/height`，与 jsdom 契约保持一致
  - [ ] spec 中不出现硬编码重复拖拽实现，统一复用 helper
...
- [x] 6. Add Window resize guardrail Playwright coverage
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `drag-only` fixture 中 `window-resize-e` 不存在，标题拖拽后 frame metrics 精确为 `{42,64,200,120}`
  - [ ] `min-clamp` fixture 中宽高各自 clamp 到 `1`
  - [ ] `max-clamp` fixture 中最终 frame metrics 精确为 `{50,50,150,110}`
...
- [x] 7. Require Playwright Window UI automation in PR CI
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `ci-pr.yml` 在 `Run unit tests` 之后新增 `npx playwright install --with-deps chromium` 与 `yarn test:ui`
  - [ ] 失败时上传 `playwright-report/` 与 `test-results/` artifact
  - [ ] 工作流仍保留原有 `lint`、`test`、`build`、`npm pack --dry-run` 步骤
...
- [x] F1. Plan Compliance Audit — oracle
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] `oracle` 输出明确 `PASS` 或 `FAIL`
  - [ ] 审计结果逐条覆盖任务 1-7 的交付物，不得只给概括性结论
  - [ ] 审计结果确认未引入计划外组件或超范围测试
...
- [x] F2. Code Quality Review — unspecified-high
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] 代码审查结果明确区分阻塞问题与非阻塞建议
  - [ ] 审查结果至少覆盖配置、helpers、spec 结构、等待策略、选择器稳定性五个方面
  - [ ] 若判定通过，结果中明确写出“无阻塞问题”或等效结论
...
- [x] F3. Real Manual QA — unspecified-high (+ playwright if UI)
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] 浏览器自动化实际访问 `http://127.0.0.1:5673/playwright-window.html`
  - [ ] 最终验收覆盖 move、content no-op、8 方向 resize、`resizable={false}`、min/max clamp
  - [ ] 产出至少一份截图或 Playwright report 作为最终验收证据
...
- [x] F4. Scope Fidelity Check — deep
  ...
  **Acceptance Criteria** (agent-executable only):
  - [ ] 范围审查结果明确说明是否存在范围膨胀
  - [ ] 若存在超范围项，结果逐条列出文件与原因
  - [ ] 若无超范围项，结果明确写出只覆盖 `Window` move / resize 及其必要 guardrail/CI 配套
